...
class SeMccbsCompactNsSpider(scrapy.Spider):
    name = 'se_mccbs_comPact_NS'
    ...
    def parse(self, response):
        ...
        characteristics = response.xpath(
            "//div[@class='drawer']/ul/li/section/table[2]/tbody")


        for n in characteristics:
            product_name = response.xpath("normalize-space(//h2[@class='pdp-product-info__description']/text())").get()
            product_height = n.xpath("normalize-space(.//tr[25]/td/div/text())").get()
            product_width = n.xpath("normalize-space(.//tr[24]/td/div/text())").get()
            product_depth = n.xpath("normalize-space(.//tr[26]/td/div/text())").get()
            product_weight = n.xpath("normalize-space(.//tr[27]/td/div/text())").get()
            yield {
...
"""         for c in characteristics:
            yield {
                'productName': response.xpath("normalize-space(//h2[@class='pdp-product-info__description']/text())").get(),
                'productHeight': c.xpath("normalize-space(.//tr[25]/td/div/text())").get(),
                'productWidth': c.xpath("normalize-space(.//tr[24]/td/div/text())").get(),
                'productDepth': c.xpath("normalize-space(.//tr[26]/td/div/text())").get(),
                'productWeight': c.xpath("normalize-space(.//tr[27]/td/div/text())").get()
            } """
